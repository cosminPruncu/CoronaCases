...
COVID.cases = {
    ...
    dropdownCountryList: function () {
        $.each(COVID.cases.country_list, function (key) {
            $(".dropdown-menu").append(`<button class="dropdown-item search-country-name">${COVID.cases.country_list[key]}</button>`);
        });
    },

    searchBarInput: function () {
        $(".input-search-country").autocomplete({
            source: COVID.cases.country_list,
            minLength: 2
        });
    },
...
$(document).ready(function () {
    ...
    COVID.cases.searchBarInput();
    COVID.cases.bindSearchOnSearchButton();
    COVID.cases.bindSearchOnInputSearch();
});
